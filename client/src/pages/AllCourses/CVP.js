...
import CVPForm from "../../components/AllForm/CVPForm"
...
export default function CVP() {
  ...
  const getData = async () => {
    const result = await axios.get("http://localhost:3001/cvp", {params: {userID: localStorage.getItem('userID')}});
  ...
  const addData = async () => {
    const result = await axios.post("http://localhost:3001/cvp", {
  ...
  const deleteData = async (id) => {
    const result = await axios.delete(`http://localhost:3001/cvp/${id}`);
    window.location.reload();
  };
